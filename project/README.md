...
### Sub-brand lockups — MyGig Business · MyGig Worker
The two product lines share one lockup pattern (see the **Logo — sub-brand lockups** preview card):
- Wordmark · **1.5px vertical divider** at 35% opacity · sub-label
- Sub-label: **Manrope 700, uppercase, +0.22em tracking**, same height as the wordmark's x-height
- Label colour: `--brand-300` on dark backgrounds, `--brand-700` on light
- Divider height = wordmark cap-height; 18px gap each side
- Never stack the label under the wordmark, never colour the wordmark itself, never abbreviate ("BIZ", "WKR")
